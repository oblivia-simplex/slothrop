...
function Creature(chromosome::Vector, fitness::Vector{Float64})
    name = Cockatrice.Names.rand_name(4)
...
function clone_and_mutate(parent::Creature)
    ...
    offspring.name = Cockatrice.Names.rand_name(4)
end
...
mutable struct Evolution
    config::NamedTuple
    logger
    geo::Cockatrice.Geo.Geography{Creature}
    fitness::Function
    iteration::Int
...
DEFAULT_TRACE = Cockatrice.Geo.Tracer.([
  (key="fitness:1", callback=(g -> g.fitness[1])),
  (key="chromosome_len", callback=(g -> length(g.chromosome))),
  (key="num_offspring", callback=(g -> g.num_offspring)),
  (key="generation", callback=(g -> g.generation)),
 ])


function step!(evo::Evolution; crossover=crossover, eval_children=false, trace=DEFAULT_TRACE)
    ranking = Cockatrice.Geo.tournament(evo.geo, evo.fitness, trace=trace)
...
function evaluate!(evo::Evolution, fitness::Function)
  Cockatrice.Geo.evaluate!(evo.geo, fitness)
end
